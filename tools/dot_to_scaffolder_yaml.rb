...
if __FILE__ == $0 #needs to be removed if this script is distributed as part of a rubygem
  ...
  o = OptionParser.new do |opts|
    opts.banner = "
      Usage: #{SCRIPT_NAME} -g <dot_file> -f <fasta_file> -d <output_directory>
  ...
  o.parse!
  if ARGV.length != 0 or options[:reference_fasta_file].nil? or options[:dot_file].nil?
    $stderr.puts o
    exit 1
  end
  ...
  graph = GraphViz.parse(options[:dot_file])
